...
const wName = $('wName');
if (wName) {
  wName.textContent = pName ? pName : "Thositha Achari";
}
...
window.addEventListener('mousemove', (e) => {
  ...
  gsap.to(canvas, { x: x * 0.5, y: y * 0.5, duration: 0.5, ease: 'power2.out' });
  gsap.to(wishEl, { x: -x, y: -y, duration: 0.5, ease: 'power2.out' });
  gsap.to('.polaroid', { rotation: -10 + x * 0.5, x: x, y: y, duration: 0.5, ease: 'power2.out' });
});
...
function spawnFireflies() {
  ...
  for (let i = 0; i < 25; i++) {
    ...
    f.style.width = '4px';
    f.style.height = '4px';
    f.style.background = '#ffe38c';
    f.style.borderRadius = '50%';
    f.style.boxShadow = '0 0 8px #ffe38c';
    f.style.pointerEvents = 'none';
    f.style.zIndex = 20;
    document.querySelector('.scene').appendChild(f);
